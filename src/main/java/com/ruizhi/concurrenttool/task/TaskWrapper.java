package com.ruizhi.concurrenttool.task;

import com.ruizhi.concurrenttool.common.*;
import com.ruizhi.concurrenttool.exception.SkippedException;
...
public class TaskWrapper<IN, OUT> {
    ...
    public void run(ExecutorService executorService, long remainTime, Map<String, TaskWrapper> taskWrapperMap, TaskWrapper fromTaskWrapper) {
        ...
        // 没有任何依赖，立即开始执行自己的任务
        if (preTaskWrappers == null || preTaskWrappers.size() == 0) {
    ...
    private boolean isNextHasBegin() {
        if (nextTaskWrappers == null || nextTaskWrappers.size() != 1) {
    ...
    public void beginNext(ExecutorService executorService, long start, long remainTime) {
        ...
        if (nextTaskWrappers == null || nextTaskWrappers.size() == 0) {
            return;
        }
        ...
        try {
            ...
            System.err.println("beginNext wait all next task execute exception." + e.getMessage());
        }
    }
                ...
                if (wrapper.getNextWrappers() != null) {
                    for (Object next : wrapper.getNextWrappers()) {
                        if (next != null) {
                            ...
                            System.out.println(wrapper.getId() + " -> " + ((TaskWrapper) next).getId());
                        }
                    }
                }
            }
